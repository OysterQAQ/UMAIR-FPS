...
from keras.layers import Dense, concatenate, Add, Concatenate,concatenate
...
def dcnv2(input_config, spare_features_config, dense_features_config, hidden_units=[64, 32, 16], dropout_rate=0.5,
          k_reg=0.05):
    ...
    liner_output = lr_layer(concatenate(spare_features))

    dnn_input = concatenate(dense_features + embed_features)
    cross_input = concatenate(dense_features + embed_features)

    dnn_output = dnn(dnn_input, hidden_units, dropout_rate, k_reg)

    dcn_output = CrossNetMix(k_reg=k_reg)(cross_input)
    stack = Concatenate(axis=1)([dcn_output, dnn_output])
    dcn_dnn_output = Dense(1, use_bias=False)(stack)
    output = tf.math.sigmoid(Add()([dcn_dnn_output, liner_output]))
