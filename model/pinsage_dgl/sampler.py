...
class PinSAGECollator(object):
    ...
    def collate_train(self, batches):
        ...
        #采样后构造multilayer neighborhood
        # 只由heads, tails, neg_tails这些节点构成一个局部图。因为是部分节点，因此不遵循原图中item节点的编号空间，需要重新编号这个局部图中由heads→tails的边构成了pos_graph。
        # 这些边上的分数是参与pairwise loss中的positive score。这个局部图中由heads→neg_tails的边构成了neg_graph。这些边上的分数是参与pairwise loss中的negative score。
        # 注意，pos_graph与neg_graph中的节点是相同的，所以seeds = pos_graph.ndata[dgl.NID]能够代表这个局部图中的所有节点，即seeds = heads + tails + neg_tails。
        # 注意，pos_graph与neg_graph中边的数目是相同的，一条正边只与一条负边对应。这是一个缺陷，因为实践中，往往需要一条正边与多条负边比较。属于某一个batch，但是由哪一层卷积无关。
