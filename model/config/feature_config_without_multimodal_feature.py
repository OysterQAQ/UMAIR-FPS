...
def build_embed_features(embedding_dims, spare_features_config, feature_input_map, k_reg=0.01):
    embed_features = []
    embed_layer_map = {}
    for feature_name in spare_features_config:
        ...
        if embed_layer_map.get(embed_layer_name) is None:
            embedding = layers.Dense(embedding_dims, activation='relu', kernel_regularizer=regularizers.l2(k_reg))
...
# def buildLRLayer(spare_features, k_reg=0.01):
#     output = layers.Dense(1, kernel_regularizer=regularizers.l2(k_reg))(layers.concatenate(spare_features))
#     return output
#
#
# def buildDNN(dnn_input, hidden_units=[64, 32, 16], dropout_rate=0.5, k_reg=0.01):
#     x = dnn_input
#     for units in hidden_units:
#         x = layers.Dense(units, kernel_regularizer=regularizers.l2(k_reg))(x)
...
def dnn(dnn_input, hidden_units=[64, 32, 16], dropout_rate=0.5, k_reg=0.01):
    x = dnn_input
    for units in hidden_units:
