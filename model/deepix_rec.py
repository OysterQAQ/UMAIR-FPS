...
from model.config.feature_config import build_input, build_embed_features, build_spare_features, \
...
def user_aware_senet_layer(user_embedding, item_multi_modal):
    ...
    squeeze = Dense(user_embedding_dim,kernel_regularizer=regularizers.l2(1e-3))(squeeze)
    #和用户特征点乘做联系
    ...
    #再缩放到filed维度
    squeeze_with_user_embedding = Dense(filed_num,kernel_regularizer=regularizers.l2(1e-3))(squeeze_with_user_embedding)
...
def user_aware_multi_modal_cross_network(input_config, user_spare_faeture_config, user_dense_faeture_config,
                                         ...
                                         modal_dim=64, modal_embed_bn=False, mmoe=False, expert_type_list=["atten","senet"], passthrough=False, passthrough_with_split=False, atten_dropout=0.5, has_style_feature=True, dcn=True,
                                         atten=True):
    # 构建输入层（及其预处理逻辑）
    ...
    if passthrough:
        ...
        item_multi_modal = tf.stack(
            [item_spare_modal_embedding, item_semantics_modal_embedding, item_style_modal_embedding,
    ...
    if dcn:
        dnn_input = CrossNetMix(layer_num=cross_layer_num, k_reg=cross_layer_k_reg)(z)
    else:
        dnn_input = z
    # 残差链接
    if residual:
        dnn_input = Add()([dnn_input, z])
        dnn_input = BatchNormalization()(dnn_input)
        dnn_input = ReLU()(dnn_input)
    # 交叉结束后进入dnn输出
    dnn_output = dnn(dnn_input, hidden_units, dropout_rate)
